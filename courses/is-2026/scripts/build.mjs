...
for(const f of ['training.js','learning-route.js','route.css','er-guide.js','er-guide.css','product-tour.js','student-ui.css','study-steps.js','programs.js','resource-viewer.js','resources.css'])write(path.join(out,f),fs.readFileSync(path.join('src',f)));
