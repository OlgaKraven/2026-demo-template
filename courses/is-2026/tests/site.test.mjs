...
test('only verified 2026 basic level and five modules',()=>{assert.equal(c.years.length,1);assert.equal(y.year,2026);assert.equal(y.level,'basic');assert.equal(y.modules.length,5);assert.equal(y.totalSeconds,9000);assert.equal(y.assessment.maxPoints,50);assert.equal(y.assessment.criteria.reduce((s,r)=>s+r.points,0),50);ExamCore.validateCourse(c);});
