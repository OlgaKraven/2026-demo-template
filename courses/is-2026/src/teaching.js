...
(function(root){
 ...
 function guide(){
  ...
  <section><span class="section-code">01 · НАЧАЛО</span><h3>Начните с исходных документов</h3><p>Откройте «Обучение». Маршрут слева ведёт от документов к ER-схеме, базе, расчёту, приложению и сдаче. Выберите MySQL или PostgreSQL в верхнем меню. Нажмите реквизит документа, чтобы увидеть его место в модели.</p></section>
  ...
  <section><span class="section-code">03 · ПОМОЩЬ В ПРОГРАММАХ</span><h3>Найдите нужную кнопку</h3><p>В разделе «Справочник» доступны инструкции по phpMyAdmin, SQL-запросам, Visual Studio и pgAdmin. В phpMyAdmin начинайте с базы и таблиц, затем переходите к запросу. Снимок можно открыть крупнее нажатием.</p></section>
  ...
  <section><span class="section-code">05 · САМОСТОЯТЕЛЬНАЯ РАБОТА</span><h3>Тренируйтесь после разбора</h3><p>В разделе «Тренировка» выберите свой вариант V01–V30 или откройте файл задания преподавателя. Можно включить таймер, взять подсказку и поставить время на паузу. Отметки сохраняются отдельно для каждого варианта и задания. В «Пробном экзамене» выберите V01–V30: на пять модулей даётся 150 минут без паузы. Запущенный таймер продолжает идти при переходе в другие разделы.</p></section>
  ...
  <section><span class="section-code">07 · ДЛЯ ПРЕПОДАВАТЕЛЯ</span><h3>Подготовьте показ группе</h3><p>Откройте «Преподавателю», выберите модуль, СУБД и вид разбора. Здесь находятся презентация и настройка данных преподавателя. Слайды перелистываются кнопками или стрелками; «Содержание» ведёт к нужному месту. Esc закрывает окно. Ниже находится создание тренировки: заполните условие, этапы, результаты и самопроверку, добавьте файлы или ссылки. Проверьте пакет и скачайте файл задания для студентов. Профиль преподавателя и файл задания — разные форматы.</p></section>
  </div>`;closeButton();
 }
